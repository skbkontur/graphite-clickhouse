...
func testResponce(t *testing.T, step int, h *Handler, tt *testStruct, wantCachedFind bool) {
	...
	if w.Code == http.StatusOK {
		...
		cachedFind := w.HeaderMap["X-Cached-Find"]
		var wantCachedFindS []string
		if wantCachedFind {
			wantCachedFindS = append(wantCachedFindS, "true")
		}
		assert.Equalf(t, wantCachedFindS, cachedFind, "cached find mismatch, step %d", step)
