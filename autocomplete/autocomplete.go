...
func (h *Handler) ServeTags(w http.ResponseWriter, r *http.Request) {
	...
	if useCache {
		key, expr = taggedKey("tags;", h.config.Common.FindCacheConfig.FindTimeoutSec, fromDate, untilDate, r.FormValue("tag"), r.Form["expr"], "tagPrefix="+tagPrefix, limit)
...
func (h *Handler) ServeValues(w http.ResponseWriter, r *http.Request) {
	...
	if useCache {
		...
		key, expr = taggedKey("values;"+limitStr+";", h.config.Common.FindCacheConfig.FindTimeoutSec, fromDate, untilDate, r.FormValue("tag"), r.Form["expr"], "valuePrefix="+valuePrefix, limit)
		body, err = h.config.Common.FindCache.Get(key)
		if err == nil {
			// metrics.FindCacheMetrics.CacheHits.Add(1)
			findCache = true
