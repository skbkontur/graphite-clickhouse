...
func (q *query) getURL(from, until int64) (string, time.Duration) {
	duration := time.Second * time.Duration(until-from)

	n := config.BinarySearchQueryParam(q.chQueryParams, duration)
...
func (q *query) getDataPoints(ctx context.Context, cond *conditions) error {
	...
	for agg, extTableBody := range cond.extDataBodies {
		...
		go func() {
			defer data.wg.Done()
			chURL, chDataTimeout := q.getURL(cond.from, cond.until)
